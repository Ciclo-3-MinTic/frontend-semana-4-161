<template>
  <v-flex>
    <v-toolbar d-flex class="elevation-2" >
      <v-btn color="warning" fab small class="elevation-0" @click="$emit('reroll')" ><v-icon>mdi-update</v-icon></v-btn>
        <v-spacer></v-spacer>
    ...
    >
      <template v-slot:[`item.opciones`]="{ item }" >
        ...
        >
        <v-icon v-if="item.estado" color="error"  @click="$emit('desactive',item)">
